...
from linalg_3d.transformations import rotation_matrix, reflection_about_axis_matrix
...
def cube(size: float = 1.0) -> list[LineSegment]:
    ...
    # the side edges
    top_right_side = front_right_edge.transform(rotation_matrix(np.pi/2, 1, 2))
    bottom_right_side = front_right_edge.transform(rotation_matrix(-np.pi/2, 1, 2))
    right_side_edges = [top_right_side, bottom_right_side]
    left_side_edges = [edge.transform(reflection_about_axis_matrix(2)) for edge in right_side_edges]
    cube_edges = front_face + back_face + right_side_edges + left_side_edges
    return cube_edges